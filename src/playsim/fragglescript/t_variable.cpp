...
{
	return (v.type == svt_string ? atoi(v.string.c_str()) :
...
fsfix fixedvalue(const svalue_t &v)
{
	...
	v.type == svt_string ? (fsfix)(atof(v.string.c_str()) * 65536.) :
...
{
	return 
		v.type == svt_string ? atof(v.string.c_str()) :
...
DFsVariable *DFsScript::VariableForName(const char *name)
{
	...
	while(current)
    {
		if(!strcmp(name, current->Name.c_str()))        // found it?
			return current;         
